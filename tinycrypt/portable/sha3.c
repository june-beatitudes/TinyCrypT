...
rotl64 (uint64_t x, uint32_t c)
    __func_contract__ (__CPROVER_requires (0 < c % 64))
{
...
from_le64 (const uint8_t *x)
    __func_contract__ (__CPROVER_requires (__CPROVER_is_fresh (x, 8)))
{
  uint64_t u = 0x0;
  for (unsigned int i = 0; i < 8; ++i)
    __loop_contract__ (__CPROVER_loop_invariant (i <= 8)
                           __CPROVER_decreases (8 - i))
    {
      u <<= 8;
      u |= x[7 - i];
    }
...
to_le64 (uint64_t u, uint8_t *x)
    __func_contract__ (__CPROVER_requires (__CPROVER_w_ok (x, 8))
                           __CPROVER_assigns (__CPROVER_object_upto (x, 8)))
{
  for (unsigned int i = 0; i < 8; ++i)
    __loop_contract__ (__CPROVER_assigns (__CPROVER_object_upto (x, 8), i, u)
                           __CPROVER_loop_invariant (i <= 8)
                               __CPROVER_decreases (8 - i))
    {
      x[i] = u & 0xFF;
      u >>= 8;
...
static void
kp (uint8_t state[TCT_KP1600_STATE_LEN], uint8_t rounds) __func_contract__ (
            ...
            __CPROVER_requires (rounds <= 24))
{
  ...
  for (unsigned int x = 0; x < 5; ++x)
    __loop_contract__ (__CPROVER_loop_invariant (x <= 5)
                           __CPROVER_decreases (5 - x))
    {
      for (unsigned int y = 0; y < 5; ++y)
        __loop_contract__ (__CPROVER_loop_invariant (y <= 5)
                               __CPROVER_decreases (5 - y))
        {
          lanes[x][y] = from_le64 (&state[8 * (x + 5 * y)]);
        }
    }
  for (unsigned int round = 0; round < rounds; ++round)
    __loop_contract__ (__CPROVER_loop_invariant (round <= rounds)
                           __CPROVER_decreases (rounds - round))
    {
      ...
      for (unsigned int t = 0; t < 24; ++t)
        __loop_contract__ (__CPROVER_assigns (t, x, y, current,
                                              __CPROVER_object_whole (lanes))
                               __CPROVER_loop_invariant (t <= 24)
                                   __CPROVER_loop_invariant (x < 5)
                                       __CPROVER_loop_invariant (y < 5)
                                           __CPROVER_decreases (24 - t))
        {
          uint64_t tmp = x;
          x = y;
          y = (2 * tmp + 3 * y) % 5;
      ...
      uint64_t T[5];
      for (unsigned int y = 0; y < 5; ++y)
        __loop_contract__ (__CPROVER_assigns (
            __CPROVER_object_whole (T), __CPROVER_object_whole (lanes),
            y) __CPROVER_loop_invariant (y <= 5) __CPROVER_decreases (5 - y))
        {
          for (unsigned int x = 0; x < 5; ++x)
            __loop_contract__ (
          ...
          for (unsigned int x = 0; x < 5; ++x)
            __loop_contract__ (__CPROVER_assigns (
                lanes[0][y], lanes[1][y], lanes[2][y], lanes[3][y],
                lanes[4][y], x) __CPROVER_loop_invariant (x <= 5)
                                   __CPROVER_decreases (5 - x))
            {
  ...
  for (unsigned int y = 0; y < 5; ++y)
        ...
        y) __CPROVER_loop_invariant (y <= 5) __CPROVER_decreases (5 - y))
    {
      for (unsigned int x = 0; x < 5; ++x)
        __loop_contract__ (__CPROVER_assigns (
            __CPROVER_object_upto (state, TCT_KP1600_STATE_LEN),
            x) __CPROVER_loop_invariant (x <= 5) __CPROVER_decreases (5 - x))
        {
...
void
tct_shake_init (uint8_t state[TCT_KP1600_STATE_LEN]) __func_contract__ (
    __CPROVER_requires (__CPROVER_is_fresh (state, TCT_KP1600_STATE_LEN))
        __CPROVER_assigns (__CPROVER_object_upto (state,
                                                  TCT_KP1600_STATE_LEN)))
{
  for (unsigned int i = 0; i < TCT_KP1600_STATE_LEN; ++i)
    __loop_contract__ (__CPROVER_loop_invariant (i <= TCT_KP1600_STATE_LEN)
    ...
    __func_contract__ (
        __CPROVER_requires (__CPROVER_is_fresh (state, TCT_KP1600_STATE_LEN))
            __CPROVER_requires (__CPROVER_is_fresh (
                chunk, (level == TCT_SHAKE128) ? 168 : 136))
                __CPROVER_assigns (
                    __CPROVER_object_upto (state, TCT_KP1600_STATE_LEN)))
{
  const uint8_t r = (level == TCT_SHAKE128) ? 168 : 136;
  for (uint8_t i = 0; i < r; ++i)
    __loop_contract__ (__CPROVER_loop_invariant (i <= r)
                           __CPROVER_decreases (r - i))
    {
      state[i] ^= chunk[i];
    }
...
tct_shake_squeeze_destructive (uint8_t state[TCT_KP1600_STATE_LEN],
    ...
    __func_contract__ (
        __CPROVER_requires (output_len < 0xffffffffffffffff - 168)
            __CPROVER_requires (__CPROVER_is_fresh (state,
                                                    TCT_KP1600_STATE_LEN))
                __CPROVER_assigns (
                    __CPROVER_object_upto (state, TCT_KP1600_STATE_LEN))
                    __CPROVER_requires (__CPROVER_is_fresh (output,
                                                            output_len))
                                                                  ...
                                                                  output_len)))
{
  ...
  for (uint64_t i = 0; i < output_len; i += r)
    __loop_contract__ (__CPROVER_loop_invariant (i <= output_len + r)
                           __CPROVER_decreases (output_len + r - i))
    {
      for (unsigned int j = 0; j < (output_len - i) && j < r; ++j)
        __loop_contract__ (__CPROVER_loop_invariant (
            j <= (output_len - i) && j <= r) __CPROVER_decreases (r - j))
        {
          output[i + j] = state[j];
        }
      kp (state, TCT_SHAKE_ROUNDS);
